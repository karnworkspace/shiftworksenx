...
exports.deleteRosterEntry = exports.getRosterDayStats = exports.batchUpdateRosterEntries = exports.updateRosterEntry = exports.getRoster = void 0;
...
const getRoster = async (req, res) => {
    try {
        ...
        let roster = await prisma_1.prisma.roster.findUnique({
            where: {
        ...
        if (!roster) {
            ...
            roster = await prisma_1.prisma.roster.create({
                data: {
                    projectId: projectId,
                    year: yearNum,
                    month: monthNum,
        ...
        const activeStaff = await prisma_1.prisma.staff.findMany({
            where: {
                projectId: projectId,
                isActive: true,
            },
...
const updateRosterEntry = async (req, res) => {
    try {
        ...
        const validShifts = await prisma_1.prisma.shiftType.findMany({
            select: { code: true },
        });
        ...
        // Upsert roster entry
        const entry = await prisma_1.prisma.rosterEntry.upsert({
            where: {
 ...
 */
const batchUpdateRosterEntries = async (req, res) => {
    try {
        ...
        const updatedEntries = await prisma_1.prisma.$transaction(entries.map((entry) => prisma_1.prisma.rosterEntry.upsert({
            where: {
                rosterId_staffId_day: {
                    rosterId,
...
exports.batchUpdateRosterEntries = batchUpdateRosterEntries;
/**
 * Get roster statistics for a specific day
 */
const getRosterDayStats = async (req, res) => {
    try {
...
exports.getRosterDayStats = getRosterDayStats;
/**
 * Delete roster entry
 */
const deleteRosterEntry = async (req, res) => {
    try {
        const { id } = req.params;
        const entry = await prisma_1.prisma.rosterEntry.findUnique({
            where: { id },
        });
        if (!entry) {
            return res.status(404).json({ error: 'Roster entry not found' });
        }
        await prisma_1.prisma.rosterEntry.delete({
            where: { id },
        });
